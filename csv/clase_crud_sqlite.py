...
class Sqlite():
    ...
    def conectar(self):
        """
        Conecta el código con la base de datos. Devuelve el conector
        """
        try:
    ...
    def crear_tabla(self, nombre_tabla, cadena_campos):
        '''
            Crea la tabla dada como parámetro de entrada. Con los campos dados en la entrada.
            Parámetros de entrada:
                - 'nombre_tabla': nombre de la tabla a crear entre comillas.
                - cadena_campos: cadena con cada declaración de campos separados por comas.
            Devuelve el número de filas, que debería ser -1 si todo fue correctamente.
        ''' 
    ...
    def insertar_muchos_registros(self, tabla, cadena_campos, valores):
        """
            Añade una colección de registros a la tabla pasada como parámetro.
            Parámetros de entrada:
                - cadena_campos es la lista de campos 
                - valores es una lista de tuplas (return de csv.reader())
        """
        num = len(cadena_campos)
        val = 'VALUES(' + '?,'*(num-1) + '?)'
        try:
            cnx = self.conectar
            cursor = cnx.cursor()
            consulta = f"INSERT INTO {tabla} {tuple(cadena_campos)} {val}"
...
# db = Sqlite('experimento_1.db')
# db.insertar_muchos_registros()